...
Deze documentatie beschrijft de geografische indeling en de matrix-architectuur van de CI/CD-monitor op de **Abstract Foundry LumiCube**. De LumiCube bestaat uit 3 LED-panelen van elk 8x8 pixels (in totaal 192 LED's) die samen een halve kubus vormen.
...
In de onderstaande diagrammen staat de hoogste verticale index (`7` of `15`) **bovenaan** weergegeven. De lege plekken (`.`) representeren geprogrammeerde inactieve LED's die zorgen voor visuele rust en ademruimte op de kubus.
...
**Inhoud:** Alle backend pipelines (`-osrpraoa`). Elke verticale kolom representeert één complete pipeline. De eerste job in de pijplijn (index 0) start aan de voorrand van de kubus (`y:0`) en bouwt naar achteren (`y:7`) op. Kortere pipelines laten aan de achterzijde automatisch lege LED's (`.`) over.
...
**Inhoud:** `wp-tasks-osraoa` (`monitoredSpecial`) verdeeld in 4 functionele blokken + de specifieke `aoa-docker` builds. Groep 3, Groep 4 en de Docker-builds bevinden zich aan de bovenkant van het paneel (grenzend aan het top-paneel). Groep 1 bevindt zich helemaal onderaan de kubus.

```text
[y:15] .         .         .         [------- GROEP 4: GNS TESTS --------] [ DOCKER-C ]
[y:14] [----------------------]       [------- & DATABASE CLEARS ---------] [ DOCKER-B ]
[y:13] [--- GR. 3: DEPLOYS ---]       .         .         .         .         .
[y:12] .         .         .         [-----------------------------------]
...
* **Groep 2 (E2E & Environment Control):** `x: 3-6, y: 9-12`. Uitgebreide testomgevingen, e2e controlers en start/stop functionaliteiten voor test, acceptatie en productie.
* **Groep 3 (Deploys & Rebuilds):** `x: 0-1, y: 13-15`. Status van app-rebuilds en core pipeline deployments.
* **Groep 4 (GNS Tests):** `x: 3-5, y: 14-15`. Database-clears en GNS-specifieke testpakketten.
* **Docker-Builds (DOCKER):** Gekoppeld aan de uiterste rechterkolom. Bestaat uit:
...
| :--- | :--- | :--- | :--- |
| **Succeeded** | 🟩 Groen | `0x00ff00` | Statisch groen (of knippert indien een nieuwe build start) |
